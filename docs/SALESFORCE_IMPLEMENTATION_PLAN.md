...
live. The tool is **built, tested, and running against FreeWheel production for 15
regions / 153 campaigns today** — this plan covers only the Salesforce wiring, which is
configuration + credentials, not new code.
...
| 0.2 | Decide the **4 open questions** (targeting sheet vs fields; picklist upkeep; trigger mechanism; pilot region) — `SALESFORCE_FIELD_MAPPING.md` §7 | 🟨 | Answers recorded |
| 0.3 | Pick the **pilot region** (recommend USA) | 🟨 | Chosen |
| 0.4 | Confirm the **integration user** + sandbox availability | 🟦 | Named |
...
Everything else (remaining fields, poller, rollout) follows once that loop works once.

## What each side needs from the other
- **SF Admin needs from us:** `salesforce-case-fields.csv`, the Status/Reason values, the
  field-mapping doc, and the admin one-pager. All provided, generated from live config.
...
- **Picklist drift** — Campaign (153) / Brand lists change as markets are added.
